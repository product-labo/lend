...
async function sendEmail(email: string, message: string, type?: NotificationType): Promise<void> {
  ...
  const template = type
    ...
    : { subject: 'Notification from Lend', html: `<p>${message}</p>` };
    ...
    if (webhookUrl) {
      try {
        await fetch(webhookUrl, {
          ...
          body: JSON.stringify({ text: `[Lend] ${title}: ${message}` }),
        });
      } catch (err) {
